...
namespace CrystalMoon.Systems.Shaders
{
    ...
    internal class MaskShaderDrawSystem : ModSystem
    {
        ...
        {
            base.Load();
            MaskTexture = TextureRegistry.NoiseTextureSpaceStars;
        ...
        {      
            ...
            if (Draws.Count != 0)
            {
                ...
                BaseShader drawShader = Draws[0].GetMaskDrawShader();
                drawShader.Apply();

                SpriteBatch spriteBatch = Main.spriteBatch;
                spriteBatch.Begin(SpriteSortMode.Deferred, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None, Main.Rasterizer,
                    drawShader.Effect, Main.GameViewMatrix.TransformationMatrix);
        ...
        private void DrawScaledTarget(RenderTarget2D target)
        {
            ...
            var shader = MagicMothlightMaskingShader.Instance;
            shader.MaskingTexture = MaskTexture;
            shader.Effect.Parameters["uImageSize0"].SetValue(target.Size());
            shader.Apply();

            Main.spriteBatch.Begin(SpriteSortMode.Immediate, BlendState.AlphaBlend, SamplerState.PointClamp, DepthStencilState.None,
                Main.Rasterizer, shader.Effect, Main.GameViewMatrix.TransformationMatrix);
